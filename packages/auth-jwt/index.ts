...
import { IGQLAuthContext, IStorageEngine, IUser } from "models";
import { IAuthProvider, ILoginParams, IRefreshParams, ISuccessfulAuthResponse } from "auth";
import { addSecondsToDate, unixTimestamp, generateRandomID } from "shared";
...
export class AuthJWT implements IAuthProvider {
  ...
  private generateVerificationHash(hashSecret: string, userId: string | null, email: string | null, name: string | null) {
    if (!hashSecret) {
